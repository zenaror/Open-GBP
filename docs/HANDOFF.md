...
LAST PHYSICAL EVIDENCE INGESTED
  ...
  HARDWARE_TESTS §V7.5; AMENDED BEFORE HARDWARE under Issue #37) are the two-pad equivalence ON THE ENHANCED CONTROL
  CHECKER, GBP-INPUT-003: the Operator's criterion ("ambos os controles funcionam e tem que apresentar o mesmo
  comportamento") answered by machine on both ends -- RUN 19 = walk A on the ORIGINAL pad, RUN 20 = walk B on the GENERIC
  pad, completing the walk x pad matrix with RUN 17 / 18; W per key by §V7.3.9's join, S per key across the two pads, K
  the word level; his channel beside -- and NOT the ROADMAP's acceptance (a test ROM is not a game). WarioWare: TWISTED
  evaluated and REJECTED (gyroscope). The build change that would free an input run from the witness stop is ASSESSED
  in §V7.5.3 and NOT made: ~40 s is enough for the checker; a real game needs it. NOT RUN / NOT AUTHORISED HERE.
...
acceptance run with a commercial game — its PAIR is pre-registered and
AMENDED BEFORE HARDWARE (Issue #34, then #37; `HARDWARE_TESTS.md` §V7.5, RUN
19 / RUN 20, GBP-INPUT-003): on the Enhanced Control Checker, the Operator's
own criterion "ambos os controles funcionam e tem que apresentar o mesmo
comportamento" read by machine on both ends, NOT the ROADMAP's acceptance,
which stays open and unscheduled (a game that passes through the button path,
and the build change assessed in §V7.5.3); NOT RUN / NOT AUTHORISED HERE. The
next safe action is the Orchestrator's — validate the amendment, then release
Hardware Issue #36 (RUN 19 / RUN 20) to the Operator with the checklist in
chat. Whether Phase 5 then closes is an assessment step of its own (Issue
#17's precedent), and it is further away, not nearer. The
...
next        orchestrator-owned: validate #37; release Hardware Issue #36 (RUN 19 / RUN 20, the checker) to the Operator with
            the checklist in chat; after the pair, the ingestion; the real-game run and its build stay open
...
**RUN 19 and RUN 20 (GBP-INPUT-003, the Phase 5 acceptance pair on
`stream-0015`, `HARDWARE_TESTS.md` §V7.5, Issue #34) are PRE-REGISTERED and
NOT RUN; their ten names are RESERVED, verified absent on 2026-09-21, and
TAKEN even if a run aborts or RUN 20 is never executed. The console writes
the SAME names for every run (`GBP-VIDEO-004_stream-0015.*`): each run's
files are archived under its own names BEFORE the next run boots.**
...
- **That RUN 19 / RUN 20 have run, or that Phase 5 is closed, or that a run
  on the checker can close it.** The pair is PRE-REGISTERED and AMENDED
  BEFORE HARDWARE (§V7.5, Issues #34 and #37) and NOT RUN; it is the two-pad
  equivalence on the Enhanced Control Checker — the Operator's sentence,
  read in his terms by machine on both ends — and NOT the ROADMAP's
  acceptance: a test ROM is not a real game, so Phase 5 stays NOT ASSESSED
  whatever the pair shows, and its closure is further away (a game that
  passes through the button path, and the build change of §V7.5.3, are
  still needed). A pad that behaves differently there is an informative
  result about the pad or the policy, not a failed run — and nothing about
  the routing, which is FACT (§V7.4).
