...
LAST PHYSICAL EVIDENCE INGESTED
  ...
  (GBP-KEY-008) with a general payload guard; NOT executed, no run name, NOT staged. Phase 5's acceptance criterion (a real game)
  NOT assessed.
...
and not staged. The next front is the hardware checkpoint that spends
`stream-0015` — the Orchestrator designs it, the Operator runs it: a run whose
KEY lines join an instrument showing what the AGB received takes the routing
to FACT — and then Phase 5's real acceptance with a commercial game, which the
Operator schedules: the checker is a test ROM, and no game has been played
...
`dd545c01cfa99ee2437cd3a53fad44cb01439e3c794991c8cae94407373a3d49`) — NOT executed, no run name, NOT staged.** The next safe action is
the Orchestrator's: validate the checkpoint against `origin/main`
(`tests/host/test_input_keylog.py`, `tests/host/test_ringlog_payloads.py`,
GBP-KEY-010; the descriptor and the policy byte-equal to `0ff8355`; §V7
untouched), then design the hardware checkpoint that spends the candidate —
its own pre-registration, staging into `build/swiss/` under that Issue, the
instrument that shows what the AGB received, the join of the KEY lines to the
frame records — and hand it to the Operator; then Phase 5's real acceptance
with a commercial game. RUN 16 is optional and independent; nothing is
pre-registered.
...
next        orchestrator-owned: validate #27; design the hardware checkpoint that spends stream-0015 (its own
            pre-registration and staging; the instrument; the KEY-to-frame join) for the Operator; then Phase 5's
            real acceptance with a commercial game; RUN 16 optional
...
- **That `stream-0015` has run, or that the KEY record made the routing a
  ...
  actual (GBP-KEY-009, GBP-KEY-010).
