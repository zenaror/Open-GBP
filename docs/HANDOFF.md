...
LAST PHYSICAL EVIDENCE INGESTED
  ...
  (GBP-KEY-008) with a general payload guard; NOT executed, NOT staged.
  RUN 17 (walk A) / RUN 18 (walk B) PRE-REGISTERED as GBP-INPUT-002 (HARDWARE_TESTS §V7.3, Issue #28) to
  spend it: Question One answered first -- the eight OGBPFULL1 samples support no per-press join and an
  interval-wise consistency check only (within an interval the counts are not distinct), but the WHOLE-RUN
  totals of the KEY record against the decoded final tallies bind each pressed bit to its AGB key uniquely
  by the distinct counts: FACT reachable per bit, no pacing, latency out of reach. NOT RUN / NOT
  AUTHORISED HERE; stream-0014 to be preserved to build/archive/ before staging. Phase 5's acceptance
  criterion (a real game) NOT assessed and NOT pre-registered.
...
as CORROBORATED, not FACT. GBP-KEY-009 is implemented and GBP-KEY-008
repaired in the candidate `stream-0015` (`da06500`; Issue #27), not executed
and not staged; RUN 17 / RUN 18 are PRE-REGISTERED (§V7.3, Issue #28) to
spend it — the whole-run join of the KEY record to the checker's counters can
take the routing to FACT per bit (Question One answered: per-press and latency
out of reach, pacing excluded). The next front is the Hardware Issue the
Orchestrator opens after validating the pre-registration — the Operator runs
RUN 17, then RUN 18 — and then Phase 5's real acceptance with a commercial
game, which the Operator schedules and which is not pre-registered: the
checker is a test ROM, and no game has been played through the input path.
Nothing below changed for Phase 4.
...
`dd545c01cfa99ee2437cd3a53fad44cb01439e3c794991c8cae94407373a3d49`) — NOT executed, NOT staged.** **Issue #28
(2026-09-21) pre-registered RUN 17 / RUN 18 (GBP-INPUT-002,
`HARDWARE_TESTS.md` §V7.3) to spend it — NOT RUN / NOT AUTHORISED HERE.** The
next safe action is the Orchestrator's: validate the pre-registration against
`origin/main` (`tests/host/test_run17_prereg.py`; Question One's answer; the
identity `dd545c01…3a49`; the ten names absent; §V7.1 / §V7.2 untouched),
then open the Hardware Issue that moves RUN 17 to the Operator — the staging
of §V7.3.6 (stream-0014 preserved first to `build/archive/`, `make swiss`
without a rebuild, the SD copy verified by hash at
`/media/rafael/SD_GC/Open-GBP/12-stream/boot.dol` = `dd545c01…3a49`,
514 880 B) and the checklist of §V7.3.7; RUN 18 follows under its own
authorisation. RUN 16 stays optional with its stream-0014 names. The
commercial-game acceptance run is not pre-registered: the Operator's game and
a prospective definition of "reliably" come first, and a game supplies only
one end of the join.
...
next        orchestrator-owned: validate #28; open the Hardware Issue for RUN 17 (RUN 18 to follow under its own
            authorisation): preserve stream-0014, stage stream-0015, hand the checklist to the Operator; RUN 16 optional
...
`HARDWARE_TESTS.md` §V7.3, Issue #28) are PRE-REGISTERED and NOT RUN; their
ten names are RESERVED, verified absent on 2026-09-21, and TAKEN even if a
run aborts or RUN 18 is never executed. RUN 16 keeps its number and its
stream-0014 names (the numbering resolution of §V7.3.2: a reserved number
belongs to its experiment whenever it runs; a new run takes the next number
above every reserved one). The console writes the SAME names for both
(`GBP-VIDEO-004_stream-0015.*`): each run's files are archived under its own
names BEFORE the next run boots.**
...
- **That `stream-0015` has run, or that the KEY record made the routing a
  FACT.** The candidate is built and identified (`da06500`, SHA-256
  `dd545c01cfa99ee2437cd3a53fad44cb01439e3c794991c8cae94407373a3d49`), executed nowhere, staged nowhere; in Dolphin the pump slot never
  runs, so no KEY line has ever been emitted. The record makes FACT REACHABLE
  by a run that joins it to what the AGB received; only that run makes it
  actual (GBP-KEY-009, GBP-KEY-010). RUN 17 / RUN 18 are that run's
  pre-registration (§V7.3, Issue #28): NOT RUN, NOT AUTHORISED there.
...
- **That RUN 16 has run, or that RUN 14 / RUN 15 made the L/R routing a
  FACT.** RUN 14 and RUN 15 executed and are ingested (§V7.2): M = PASS, O =
  AS-ASSIGNED, both runs; RUN 16 is optional, not run, its names reserved.
  The routing stays CORROBORATED — the Operator having pressed L exactly once
  is in no machine record (GBP-HW-265); the Operator's informal, unregistered,
  incomplete trial of 2026-09-21 remains NOT a run and supports no claim
  (§V7.1.2).
...
- **That the input path is a finished feature because RUN 14 / RUN 15
  passed.** `stream-0014` (`0ff8355`, SHA-256 `ef76a170…`) ran twice with one
  instrument, a counting test ROM, one GENERIC third-party pad in port 1 (its
  digital click; the official pad owned and NOT used, Issue #25), one cartridge; every pressed button reached its own counter (GBP-HW-264,
  GBP-HW-265). Nothing was measured about latency, about the 5 ms refresh
  being needed, about other pads, ports or cartridges, or about a real game;
  the descriptor's assignment is CORROBORATED data that survived its
  falsifier, not a physical FACT (GBP-KEY-006, GBP-KEY-009).
- **That the KEYPAD L/R routing is a physical FACT.** The Start-up Disc, GBI
  and Dolphin's model all write L at word bit 8 and R at bit 9 (the reverse
  of KEYINPUT), and RUN 14 / RUN 15 read the checker's L at 1 and R at 2 under
  ...
  would carry it — the KEY line of `stream-0015`, implemented under Issue #27
  and executed nowhere yet (GBP-KEY-009, GBP-KEY-010). `REGISTERS.md`, `GBS-DOL.md`, `ARCHITECTURE.md`
...
- **That the keypad plane inherits the video plane's confidence.** It does
  ...
  (GBP-KEY-001…005, GBP-VID-011); the physical record now holds exactly two
  runs of one instrument (GBP-HW-261…265) and nothing else.
