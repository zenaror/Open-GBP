...
LAST PHYSICAL EVIDENCE INGESTED
  ...
  Phase 5's acceptance criterion (a real game) NOT assessed. Its run IS pre-registered (Issue #34, HARDWARE_TESTS
  §V7.5): RUN 19 (generic pad) / RUN 20 (original pad), GBP-INPUT-003 -- the criterion in the Operator's words ("ambos os
  controles funcionam e tem que apresentar o mesmo comportamento"), a pair with the same 13-step / 19-press action list,
  W / S read from his per-step reports, K the machine half from the two KEY records beside them; stream-0015 serves
  (the KEY record; the routing FACT on both pads; the ~40 s window); the cartridge and its status (original /
  unofficial / a ROM from the EZ-Flash) declared before each run; NOT RUN / NOT AUTHORISED HERE; Phase 5's closure
  NOT decided by it.
...
acceptance run with a commercial game — now PRE-REGISTERED (Issue #34,
`HARDWARE_TESTS.md` §V7.5, RUN 19 / RUN 20, GBP-INPUT-003; the Operator's own
criterion "ambos os controles funcionam e tem que apresentar o mesmo
comportamento", fixed before any data; NOT RUN / NOT AUTHORISED HERE): the
next safe action is the Orchestrator's — validate §V7.5, then open the
Hardware Issue that moves RUN 19 and RUN 20 to the Operator with the checklist
in chat; the cartridge and its status are his to declare before each run.
Whether Phase 5 then closes is an assessment step of its own (Issue #17's
precedent), after the pair. The
...
next        orchestrator-owned: validate #34; open the Hardware Issue for RUN 19 / RUN 20 with the checklist in chat; after
            the pair, the Phase 5 assessment as its own step
...
- **That RUN 19 / RUN 20 have run, or that Phase 5 is closed.** The
  acceptance pair is PRE-REGISTERED (§V7.5, Issue #34) and NOT RUN; the
  criterion is the Operator's sentence and is read in his terms after the
  pair; whether the phase closes is an assessment step of its own, after the
  result, never anticipated (Issue #17's precedent). A pad that behaves
  differently there is an informative result about the pad or the policy,
  not a failed run — and nothing about the routing, which is FACT (§V7.4).
