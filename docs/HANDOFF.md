...
issue 59    THE IMAGE §V8 NEEDS IS BUILT: poc/gbp-audio-window-probe, app gbp-audio-window-probe, BUILD
            stream-0016, DOL 498 496 B. HOST-VALIDATED, NEVER RUN, NOT AUTHORISED, NOT STAGED (12-stream and
            13-play keep their frozen hashes; the layout has no row for it). play-0001's runtime PLUS the
            retention and the emission. THE SERVICE PATH GAINS ONE OPTIONAL FIELD (cfg->awin) AND ONE CALL,
            after the drain and its commit: at most one 4096-byte memcpy in RAM, measured between two now32
            reads, the witness hook's shape since §V5.39 -- NO device operation added, removed or reordered,
            proved by diffing the module against the Issue #19 base. THE SUBTRACTION IS A DIFF: input_step,
            session_step, submit_ready and offer_oldest_ready are character-for-character play-0001's;
            keylog_emit and pump gained EXACTLY ONE LINE each, both named in the test. THE ANCHOR IS A RISING
            EDGE, not a key event -- a release must not consume one of the four windows. THE WINDOW IS §V8's:
            4 x 256 press blocks PLUS a control window of the same size (§V8.5.1), 5 242 880 B, asserted at
            compile time with "a build may not economise". THE BUDGET IS STATED IN ADVANCE (play-0001's own
            GBP-HW-282 lesson): safety 120 s; frame store 16384 = the contract's MINIMUM and 274 s; event
            store 8192, ABOVE the contract's 4096 because the archive's rate for it is the FRAME rate
            (59.81/s) and 4096 would have bounded the session at 68 s INSIDE the budget; 6 029 312 B given
            up, 5 242 880 B taken, and the ELF is 775 104 B SMALLER than play-0001's. UNMEASURABLE HERE: a
            session over 120 s and the video research instrumentation. NEW CONTRACT OGBPAW1: one 128-byte
            anchor per window (the KEY event's n, the word, the three ticks64 instants, the first/last
            delivery index, the flags), then the blocks, then a footer CRC; the 5 MB is never copied through
...
issue 81    THE AUDIO DECODE IS RUNTIME CODE, proved on the bytes already owned; NO HARDWARE, NO DOLPHIN,
            NOTHING PROMOTED. FIXTURES: RUN 33 and RUN 34's raw sidecars versioned as gzip -9 mtime=0
            (captures/fixtures/...-run33-audio.bin.gz / -run34-, 22 338 / 27 037 B), identified by the raw
            log's and the raw sidecar's size and hash (captures/README.md); recompressing reproduces them
            byte for byte. THE FIRST FIXTURES OF ANY RUN AFTER RUN 18. src/audio/: gbp_adec (H-PWM decode
            in integers -- round_half_even((n*p - S) * 32767 / (5*1024*n)), no float, caller-owned ring,
            overflow and lost-block counters, a lost block HELD so the 4096 Hz timebase is kept),
            gbp_asrc (the source boundary + a replay backend over gbp_awindump_parse), gbp_aresamp
            (4096 -> 32 000 = 125/16 by an INTEGER phase accumulator: after j inputs exactly
            ceil(125j/16) outputs, 16 taps, Q15 rows summing to 32768, table GENERATED by
            tools/gen_aresamp.py and checked against it). BIT-IDENTICAL to tools/v17decode.py on all ten
            windows (the eight press windows and both controls, 2 560 samples) and to #80's WAVs frame
            for frame. THE 1024 Hz WINDOW, SAID PLAINLY: its fundamental comes through at 1.0007
            (+0.006 dB); its PEAK comes out 25 % ABOVE the largest source sample, because four samples
            per period miss the sine's crest -- a peak comparison can only ever read a GAIN there (up to
            41 %), so the test measures the fundamental. A BUG CAUGHT BEFORE SHIPPING: phase 0's tap is
            32768, which an int16 table would have wrapped to -32768, inverting every output that lands
...
**Not every run has one.** The paragraphs above were true through RUN 18. No run
after it has a versioned fixture **except RUN 33 and RUN 34's audio sidecars**,
versioned by Issue #81 (`captures/README.md`) so the runtime decoder replays from
the repository; every other later run is identified by hash in
`docs/research/HARDWARE_TESTS.md` and recomputable only where its raws are.
