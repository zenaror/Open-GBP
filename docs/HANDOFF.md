...
`…-run11…` names, which are now taken.** No run is pre-registered; the next one
must reserve its names here BEFORE the hardware. An embedded build id does not
mean the same physical run.
