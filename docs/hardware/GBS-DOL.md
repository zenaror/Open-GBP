...
| Keypad injection | KEYPAD (16-bit in bytes 0x1E–0x1F, 1 = pressed; bits 0–7 in the GBA KEYINPUT order; **bit 8 = L, bit 9 = R** — the reverse of KEYINPUT's, as the Start-up Disc, GBI and Dolphin's model all write it) | refreshed on every IRQ by the Start-up Disc (and every 5 ms) and by GBI; `0x0304` used to wake from sleep (GBI). **Open-GBP wrote it on hardware on 2026-09-21 (GBP-INPUT-001, RUN 14 / RUN 15): 7 892 / 7 895 completed writes, every pressed button counted at its own counter by the test ROM, L = 1 and R = 2 under that assignment — through the digital click of a generic third-party pad** (GBP-HW-262, GBP-HW-264, GBP-HW-265) | C (format, polarity: GBP-KEY-001, GBP-KEY-005); F (hw, run-scoped) that a written word reaches the cartridge as presses; **C (L/R order)** — H until 2026-09-21, not FACT: the chain rests on the Operator's press count, GBP-KEY-009 names the log line that would close it, and the official pad was not exercised (GBP-HW-261); details `docs/protocol/INPUT.md` |
