...
| Streams (2026-09-25, Issue #123) | Read as 32 groups of 8 bytes, a slice is eight stride-8 streams (byte j of every group, most significant bit first). Streams 1 and 3 are equal (A) and so are 5 and 7 (B); each is exactly **one run of ones** per slice; every even stream is a superset of its odd partner (the extras). The one-bit count is 4 wA + 4 wB + extras. So the level per stream is an integer 0..256 per slice, free of the extras. A == B in every tone slice and in 12.4 % of a game's: that A and B are the two output sides is a HYPOTHESIS (`U-GBP-047`). | F (counted: every slice of RUN 33, RUN 34 and RUN 43's raw window); the sides: HYPOTHESIS | GBP-HW-347 |
| Update grid (2026-09-25, Issue #123) | The tones change level only between slice PAIRS; a game changes inside 15.8 % of them. A decode per slice per stream is exact at both grids; a decode per pair averages two of a game's samples. The share of pairs whose slices differ reads the grid from the content. Why the tones hold one pulse per 256 cycles when nothing set SOUNDBIAS is `U-GBP-048`. | F (counted, three captures); the Hz, 32 768 against 65 536 updates a second, conditional on uniform slices (`U-GBP-041`) | GBP-HW-348 |
...
| Slice decode (2026-09-24, Issue #118) | Decoded by slice PAIRS instead of one value per block, RUN 33's and RUN 34's blocks are the programmed square waves sampled at 32 768/s nominal (correlation ≥ 0.99998; the same odd harmonics to the 127th; 2.5–18.7 % of each tone's energy above 2 048 Hz). The one-value-per-block decode has its Nyquist at 2 048 Hz, so none of that energy can be in it; and being a boxcar-and-decimate with no anti-alias stage, it folds part of it onto in-band bins (at 512 Hz its |H3|/|H1| is 0.263 against 0.334). For these tones the bytes hold at least 8× the bandwidth the runtime decodes. **No game's blocks exist in the archive.** **2026-09-25, Issue #123:** RUN 43's raw window is a game's blocks (`GBP-HW-345`), and a game updates on single slices: the decode that is exact for tones and game alike is per SLICE per stream (`GBP-HW-347`, `GBP-HW-348`). | F (arithmetic on the archive); the Hz axis is conditional on uniform slices (`U-GBP-041`) | GBP-HW-340; the cost, in today's units, is there too |
...
- **`U-GBP-041`:** whether the sixteen slices are uniform in time, and whether the
  ...
  32 768/s PWM rate is a HYPOTHESIS inside `GBP-HW-315`. **2026-09-25:** the grid is the source's, pairs for the tones and single slices for a game (`GBP-HW-345`, `GBP-HW-348`).
...
- **`U-GBP-047`:** whether streams A and B are the AGB's two output sides, and which is left (2026-09-25).
- **`U-GBP-048`:** why the tones hold one pulse per 256 cycles when nothing set SOUNDBIAS (2026-09-25).
...
- whether the one pulse per block is the AGB's left, right or mixed output; numbered since 2026-09-25: `U-GBP-047`, above;
