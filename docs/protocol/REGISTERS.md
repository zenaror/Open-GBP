...
| 15 | 0x8000 | written 1 at IRQ entry (`mask \| 0x8000`) and at stop, 0 at start / exit / DMA done; never tested | written 1 after reading (`read \| 0x8000`), 0 with `IRQ := 0`; never tested | `IRQ_ASSERTED`: set with any source, W1C, drives the line | Not a pair with bit 14. **Hardware 2026-09-15 (GBP-HW-029/032):** written 0 (A2) it read 0 for ≥ 50 ms with nothing pending; written 1 (stop word) it read 1 with nothing pending — writable and persistent both ways, **F**; the PI cause arrived while it read 0. Reading (ii) "pending summary, W1C" is therefore **rejected**; reading (i) "global hold/mask, level, 1 = held" stays **H** (never isolated from the odd bits). Dolphin's `irq & 0x8000` assertion condition does not match the hardware |
...
|---------|--------------------|----------|--------|
| `0xCC003000` | PI INTSR (interrupt **cause**) | bit 13 (0x2000) = HSP; bit 16 = reset-switch state. **Hardware 2026-09-15 (GBP-HW-030/033, GBP-PI-004):** bit 13 read 1 while INTMR bit 13 was 0 (captured independently of the mask, no CPU exception), stayed 1 after the device-side sources had been cleared (latched at the PI), and one `INTSR := 0x2000` cleared it — **F** for bit 13. Every INTSR write in libogc2, the SDK, GBI and the Disc is such a single-source acknowledge: 2 (reset switch), 0x1000 (debugger), 0x2000 (HSP). Still **U**: level/pulse nature of the GBS-DOL line and W1C while the device still asserts (U-GBP-022) | F (bit 13) / U (line) |
