...
| 0xC | KEYPAD | W | 16-bit at bytes 0x1E–0x1F, 1 = pressed; bits 0–7 = A, B, Select, Start, Right, Left, Up, Down (the KEYINPUT order); bit 8 = L, bit 9 = R as every reference writes it (§2.3) | written on every HSP IRQ and every 5 ms tick | written on every IRQ (with IRQ ack in the same 64-byte block); 0 at start; `0x0304` (= L+R+Select) on sleep IRQ | lo byte = GBA keys 0–7; hi bit0→L(key 9), bit1→R(key 8) | C (existence/format; polarity C, GBP-KEY-005). **F (hw, 2026-09-21, run-scoped):** written by Open-GBP on this hardware in RUN 14 / RUN 15 — 7 892 / 7 895 completed 32-byte writes, 0 failed — and the written word reached the cartridge as presses, every pressed button at its own counter (GBP-HW-262, GBP-HW-264, GBP-HW-265). **L/R bit order: C — was H until 2026-09-21** (the Disc, GBI and Dolphin's model agree, GBP-KEY-004; RUN 14 / RUN 15 read L = 1, R = 2 under that assignment, U-GBP-010 CLOSED), **not FACT**: the chain rests on the Operator's press count, not on a machine record (GBP-KEY-009 names the log line that would close it), and the observation passed through the digital click of a generic third-party pad — the official pad was not exercised (GBP-HW-261). Details `INPUT.md` | GBP-KEY-001, GBP-KEY-004, GBP-KEY-005, GBP-KEY-006, GBP-HW-261, GBP-HW-262, GBP-HW-264, GBP-HW-265 |
...
|---|---|---|
| 0–7 | A, B, Select, Start, Right, Left, Up, Down (the KEYINPUT order), 1 = pressed — the opposite polarity of the AGB's KEYINPUT | C (every reference writes it so, GBP-KEY-002, GBP-KEY-003; the polarity and bits 4–7 corroborated from the AGB side, GBP-KEY-005; in RUN 14 / RUN 15 each pressed button was counted at its own counter, GBP-HW-264, GBP-HW-265) |
| 8, 9 | bit 8 = L, bit 9 = R — the reverse of KEYINPUT's | C, not FACT (was H until 2026-09-21): the Disc, GBI and Dolphin's model agree (GBP-KEY-004) and RUN 14 / RUN 15 read L = 1, R = 2 under this assignment through the digital click of a generic third-party pad (GBP-HW-263, GBP-HW-264, GBP-HW-265; the official pad not exercised, GBP-HW-261); the chain rests on the Operator's press count, not on a machine record — GBP-KEY-009 names what would close it; U-GBP-010 CLOSED |
