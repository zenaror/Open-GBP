...
| 0xC | KEYPAD | W | 16-bit at bytes 0x1E–0x1F, 1 = pressed; bits 0–7 = A, B, Select, Start, Right, Left, Up, Down (the KEYINPUT order); bit 8 = L, bit 9 = R as every reference writes it (§2.3) | written on every HSP IRQ and every 5 ms tick | written on every IRQ (with IRQ ack in the same 64-byte block); 0 at start; `0x0304` (= L+R+Select) on sleep IRQ | lo byte = GBA keys 0–7; hi bit0→L(key 9), bit1→R(key 8) | C (existence/format; polarity C, GBP-KEY-005). **F (hw, 2026-09-21, run-scoped):** written by Open-GBP on this hardware in RUN 14 / RUN 15 — 7 892 / 7 895 completed 32-byte writes, 0 failed — and the written word reached the cartridge as presses, every pressed button at its own counter (GBP-HW-262, GBP-HW-264, GBP-HW-265). **L/R bit order: F (hw, run-scoped) since 2026-09-21** — RUN 17 (the generic third-party pad) and RUN 18 (the original Nintendo pad) on `stream-0015` bound bit 8 to L and bit 9 to R by the machine join of the runtime's own KEY record (the word sent at each change, GBP-KEY-010) to the checker's decoded counters, no human count in the chain (GBP-HW-267, GBP-HW-269, GBP-HW-270; `HARDWARE_TESTS.md` §V7.4). History, kept: **C — was H until 2026-09-21** — from the promotion of that day until the join later the same day (the Disc, GBI and Dolphin's model agree, GBP-KEY-004; RUN 14 / RUN 15 read L = 1, R = 2 under that assignment, U-GBP-010 CLOSED), **then not FACT**: the chain rested on the Operator's press count, not on a machine record (GBP-KEY-009 named the log line that would close it), and the observation had passed through the digital click of a generic third-party pad only (GBP-HW-261). Details `INPUT.md` | GBP-KEY-001, GBP-KEY-004, GBP-KEY-005, GBP-KEY-006, GBP-KEY-010, GBP-HW-261, GBP-HW-262, GBP-HW-264, GBP-HW-265, GBP-HW-267, GBP-HW-269, GBP-HW-270 |
...
| bit 15 of the 16-bit color on the **first pixel** of a 0xF00 block | first block of a frame: block index := 0 | C (DOLPHIN sets it; GBI tests `(w & 0x80800000) == 0x80800000` = bytes 0 and 1; DISC tests `(hw >> 7) & 1` = byte 1 and pre-fills its dummy first block with `halfword |= 0x0080`; physical first block: set — GBP-VID-004). **Physically: exactly one consumed word per frame carries the bit, at pixel (0, 0), in every frame captured, and the AGB wrote 0 there, so it is added on the path — F (hw), GBP-HW-129, GBP-HW-077; what sets it is open, U-GBP-034** |
| color encoding | **bits 14–10 R, 9–5 G, 4–0 B (GX RGB5A3 order)**, and the path **exchanges the two outer 5-bit groups** relative to the AGB framebuffer: an AGB value of `0x001F` (its own blue) arrives as `0x7C00`, `0x0001` arrives as `0x0400`, bits 9–5 unchanged. Measured directly with a controlled eight-bar Mode 3 stimulus: GBP-VIDEO-003 `color-0002`, `CONFIRMED_EXACT_H1_OUTER_GROUP_SWAP` under a contract pre-registered before the run (§V4), exactly one of seven candidate transformations reproducing all eight known values; reproduced by `color-0001`. The references therefore render the AGB's intended colour, and DOLPHIN's mGBA macro (GBA order, bits 0–4 R) is the divergent model. **Limit:** the stimulus pins bits 0, 5 and 10 individually and each group as a set; a permutation fixing those three while rearranging only bits 1–4 inside a group is not excluded (§V3.19) | **F (hw)**, U-GBP-011 CLOSED 2026-09-18; GBP-HW-130, GBP-HW-131 |
| geometry of a block | 4 raster lines × 240 pixels × 4 bytes, line stride 960 bytes; 40 blocks per 160-line frame; block `i` → lines `4i..4i+3` | C (DISC conversion loop, GBI copy + tiler, DOLPHIN) — GBP-VID-002/005; **F (hw)**: reconstructing preserved raw frames with exactly this geometry yields a legible, animated 240 × 160 logotype (GBP-HW-081), and 477 of 489 measured frame intervals are exactly 40 blocks (GBP-HW-076); the consolidated video-path reference is `VIDEO.md` |

### 2.3 KEYPAD word (written; consolidated in `INPUT.md` after GBP-INPUT-001, 2026-09-21)

| Bits (16-bit word at bytes 0x1E–0x1F) | Meaning | Status |
|---|---|---|
| 0–7 | A, B, Select, Start, Right, Left, Up, Down (the KEYINPUT order), 1 = pressed — the opposite polarity of the AGB's KEYINPUT | **F (hw, run-scoped) since 2026-09-21** — RUN 17 bound bits 0–3 (A, B, SELECT, START) and RUN 18 bits 4–7 (RIGHT, LEFT, UP, DOWN) each to its key by the machine join (GBP-HW-270); before it C (every reference writes it so, GBP-KEY-002, GBP-KEY-003; the polarity and bits 4–7 corroborated from the AGB side, GBP-KEY-005; in RUN 14 / RUN 15 each pressed button was counted at its own counter, GBP-HW-264, GBP-HW-265) |
| 8, 9 | bit 8 = L, bit 9 = R — the reverse of KEYINPUT's | **F (hw, run-scoped) since 2026-09-21** — RUN 17 (generic pad) and RUN 18 (original pad) bound bit 8 to L and bit 9 to R by the machine join of the KEY record to the checker's counters, no human count in the chain (GBP-HW-270, GBP-HW-267, GBP-HW-269; §V7.4). History — C, not FACT (was H until 2026-09-21), from the promotion of that day until the join: the Disc, GBI and Dolphin's model agree (GBP-KEY-004) and RUN 14 / RUN 15 read L = 1, R = 2 under this assignment through the digital click of a generic third-party pad (GBP-HW-263, GBP-HW-264, GBP-HW-265; the official pad not exercised then, GBP-HW-261); the chain then rested on the Operator's press count — GBP-KEY-009 named what would close it, GBP-KEY-010 implemented it, RUN 17 / RUN 18 closed it; U-GBP-010 CLOSED |
| 10–15 | never set by any reference; written 0 by Open-GBP | F (static), GBP-KEY-002, GBP-KEY-003; F (software), GBP-KEY-006 |
...
|----:|-----------|-----------|----------------------|--------|
| 0x01 | read → status flag "type" | read → selects "Game Boy" vs "Game Boy Advance" strings | `CART_IS_GB` (1 = GB/GBC game pak) | C (usage). **F (hw, four runs, three cartridges, 2026-09-22):** with GB/GBC media in the slot the bit **reads 1 after the transform write and holds to the end of the run**; with a GBA cartridge or an empty slot it reads 0 throughout (GBP-HW-275 with its amendment; RUN 24 / 27 / 28 / 29, two media families). **THE BIT IS NOT IN THE ORIGINAL BYTE** — sampled before the write it is `0x92`, indistinguishable from a GBA cartridge (GBP-HW-274). **Scope:** one console, one Game Boy Player, this read sequence. **WHEN it arrives is CARTRIDGE-DEPENDENT** — 186–636 µs for three ordinary cartridges, 698–1148 µs for an Everdrive, disjoint windows — which excludes elapsed time alone and the `A1` write alone, and is why the FACT is worded as what was measured rather than as "reports the type". **It does NOT distinguish DMG from CGB.** Why it arrives late is UNKNOWN (U-GBP-036, narrowed) |
