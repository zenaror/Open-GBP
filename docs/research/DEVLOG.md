...
## 2026-09-23 — Issue #81: the AUDIO decode becomes runtime code, bit-identical to #80

**Goal.** Make #80's decode runtime code and prove it on the bytes we already own,
with no console, no emulator, and nothing about the continuous drain.

**The fixtures.** RUN 33 and RUN 34 are no longer logs someone has. Each raw
OGBPAW1 sidecar is versioned under `captures/fixtures/` as gzip -9 with a zero
mtime — 22 338 and 27 037 bytes for 5.2 MB each — identified by the raw log's and
the raw sidecar's size and hash, and **recompressing the content reproduces the
`.gz` byte for byte**, so the fixture is a function of the raw and nothing else.
Only raw bytes are stored; the decoded samples are derived, never versioned. They
are the first fixtures of any run after RUN 18 — `HANDOFF.md` had said every run
kept one, and now says which do.

**The decoder** (`src/audio/gbp_adec.c`) is H-PWM in integers:
`round_half_even((n·p − S) · 32767 / (5 · 1024 · n))`, with p the block's one-bit
count and S / n the calibration span's. That is v17decode's float expression with
the float removed, and its half-to-even rounding is Python's — which is why the
result is identical rather than close. No allocation (the ring is the caller's),
no blocking call, a full ring **drops and counts**, and a lost block is **held
and counted** so the 4096 Hz timebase survives it. The replay backend
(`gbp_asrc.c`) serves a sidecar window by window through the existing strict
parser. **Bit-identical on all ten windows** — the eight press windows #80
decoded and both control windows, 2 560 samples — against `v17decode.write_wav`
run on the fixtures, and against #80's own WAVs frame for frame. Two mutations
of the decoder (the divisor, and the rest off by one bit per block) are caught
by the same test, with 237 and 256 samples differing.

**The resampler** (`gbp_aresamp.c`): 32000/4096 = 125/16, walked by an integer
accumulator (+16 per output, −125 per input). Output k is emitted by input
⌊16k/125⌋, so after j inputs exactly ⌈125j/16⌉ outputs exist — an identity with
no remainder carried, tested at every j, and after 4096 inputs the phase is home.
16-tap windowed sinc, cutoff at the input Nyquist, one Q15 row per phase summing
to exactly 32768 (DC gain exactly 1), generated by `tools/gen_aresamp.py` and
checked against it. Passband within 0.01 dB from 128 to 1024 Hz. Latency 8
inputs.

**The 1024 Hz window, said plainly.** Its fundamental comes through at 1.0007
(+0.006 dB). Its **peak comes out 25 % above the largest source sample**, not
below it: four samples per period miss the crest of the sine they sample (the
largest sample is cos 36.9° = 0.80 of the amplitude here), and a band-limited
reconstruction restores the crest. A peak comparison can only ever read a *gain*
at four samples per period — up to 41 % — so the test measures the fundamental.
The window's Nyquist term is zero, as the half-wave symmetry §V17.6.1 recorded
requires; nothing new about the hardware. I had written the opposite direction
into the header comment before measuring; the measurement corrected it before
the commit.

**A bug caught before it shipped.** Phase 0's single tap is 1.0 = 32768 in Q15,
one more than INT16_MAX. The table was first declared `int16_t`, which would have
wrapped it to −32768 and inverted every output landing on an input sample. It is
`int32_t` now, the generator asserts the range of the type it emits, and a test
pins that the table does need it.

**A test-harness defect caught too.** The host test first reported a compile
failure as "gcc unavailable" and skipped. It now skips only when there is no
compiler, and fails when there is one and the harness does not build.

**Not established, and not claimed.** Re-deriving a known result in new code
establishes nothing about the hardware: EVIDENCE.md is untouched. **Out of scope
and not started:** the AI plumbing and whether the runtime can drain 4096
blocks/s continuously without loss — the real hardware question, which gets its
own POC and pre-registration. No hardware, no Dolphin.
