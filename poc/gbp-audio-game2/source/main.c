/*
 * Open-GBP GBP-AUDIO-011, build game-0002 — PHASE 6's ACCEPTANCE, SECOND ATTEMPT: the start-up clause
 * rebuilt on the evidence, and the one record that locates the frame (GitHub Issue #113; HARDWARE_TESTS
 * §V26, with §V26.7's decisions and §V26.8's confirmed readings). IMPLEMENTED AND HOST-VALIDATED; NOT
 * PHYSICALLY EXECUTED; staging and the run belong to a separate Hardware Issue.
 *
 * WHAT THIS IS (§V26.2, ONE VARIABLE, READ-ONLY). game-0001 -- RUN 41's image, poc/gbp-audio-game at
 * 6129104, whose own header is below this one -- with one record added after the session and nothing
 * else changed but its identity. tests/host/test_game2_image.py shows it by diffing.
 *
 *   GAME 6  THE BEFORE-AI FRAMES (§V26.7 1 & 2, readings (s2), (s5), (s6)): after the session, off the
 *           drain path, the frame store is walked once more and every before-AI incomplete frame -- the
 *           predicate LIVEVINC's `before` counts -- gets one LIVEVBEF line: its store index, its
 *           t_first_block, its t_last_block and the NEXT stored frame's t_first_block. At most 64 lines;
 *           LIVEVBEFN says how many there were, how many are listed, and whether the list was capped.
 *
 * The verdicts are not computed here: tools/v26report.py turns the log into the report tools/v26accept.py
 * reads, and that tool was frozen (f874cc6) before this image existed.
 */
/*
...
#define OPENGBP_APP_NAME "gbp-audio-game2"
#endif
...
static void live_screen(uint64_t now)
{
    if (!live_drawn_prompt && live.t_accept && now >= live.t_accept && live.phase == GBP_ALIVE_PROMPT) {
        live_drawn_prompt = 1;
        printf("\x1b[8;0H");
        printf("  >>> PRESS A when ready: the game appears. PLAY ~65 s. Do NOT hold Z. <<<        "); /* GAME 3 */
    }
    if (!live_drawn_press && live.phase == GBP_ALIVE_DELAY) {       /* GAME 3: the press starts the delay */
        ...
        live_drawn_press = 1;
        printf("\x1b[10;0H");
        printf("  A RECEIVED. The game appears now: PLAY it, any buttons, for about 65 s.         "); /* GAME 3 */
        printf("\x1b[11;0H");
        printf("  Do NOT hold Z. The report appears when the game disappears.                     "); /* GAME 3 */
    }
}
...
static void live_screen_report(const struct gbp_vstate_result *res)
{
    ...
    /* GAME 5 (Issue #110): no positive control runs; the decoder's clips and spread; the video's location */
    printf("  ORIGIN  the A press + %u ms; no positive control (HARDWARE_TESTS V25)\n", (unsigned)GAME_ORIGIN_DELAY_MS);
    printf("  DECODE  clipped %lu samples; calibration %lu blocks, one-bit count %lu..%lu\n",
           (unsigned long)adec.clipped, (unsigned long)adec.rest_n, (unsigned long)adec.rest_min,
           (unsigned long)adec.rest_max);
    printf("  VIDEO   incomplete frames %lu before / %lu inside / %lu after the AI span (start-up signature 13)\n",
           (unsigned long)game_v_before, (unsigned long)game_v_inside, (unsigned long)game_v_after);
    ...
    printf("  L2      %s: %lu chunks kept, %lu in the window (%lu silence). Verdicts: tools/v25accept.py.\n", /* GAME 5 */
...
int main(void)
{
    ...
    printf("  WAIT for the prompt below (about 5 s). Then press A when ready: the game appears.\n");
    printf("  PLAY it for about 65 s, any buttons. Do NOT HOLD Z: a 1/4 s hold ends the session.\n");
    printf("  X does nothing until the end. The report appears when the game disappears.\n");
