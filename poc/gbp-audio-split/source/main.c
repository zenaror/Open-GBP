/*
 * Open-GBP GBP-AUDIO-009, build split-0001 — RUN B: split `produce`, hold total work constant,
 * and separate CAUSE from ACCOMMODATION (GitHub Issue #105; HARDWARE_TESTS §V24, with §V24.7's
 * confirmed readings). IMPLEMENTED AND HOST-VALIDATED; NOT PHYSICALLY EXECUTED; staging and the
 * run belong to a separate Hardware Issue.
 *
 * WHAT THIS IS (§V24.1, ONE VARIABLE, INTERLEAVED WITHIN THE RUN). trace-0001 -- RUN 39's image,
 * poc/gbp-audio-trace at c1beea1, whose own header is below this one -- with each AI chunk
 * produced in 16-push or 8-push steps and nothing else changed but its identity. Every added or
 * changed line sits in a block marked SPLIT; tests/host/test_split_image.py shows it by diffing.
 *
 *   SPLIT 1  the gbp_aplay step-size hook (NULL in every earlier build) is src/audio/gbp_asplit:
 *            the frozen pair-balanced assignment, xorshift32 from 0x9E3779B9 (§V24.7 (r6)).
 *            Total work per chunk is identical: the same samples, the same arithmetic, the same
 *            DUP/DROP decision, taken at the chunk's start. Only the partition into calls moves.
 *   SPLIT 2  every production step's record carries its chunk's APPLIED arm, a first-step mark
 *            and the chunk's seq mod 64 in its spare byte (§V24.7 (r5)); the host regenerates the
 *            assignment and refuses the run if they disagree
 *   SPLIT 3  §V24.4's floorless sample: every chain step of every 8th AI cycle, with its tag
 *   SPLIT 4  the log's LIVESPLIT record: the assignment as applied and the sample's counts
 *
 * The verdicts are not computed here: tools/v24report.py turns the log and the trace into the
 * report tools/v24accept.py reads, and that tool was frozen before this image existed.
 */
/*
...
#include "gbp_asplit.h"                 /* SPLIT (Issue #105, §V24) */

#ifndef OPENGBP_APP_NAME
#define OPENGBP_APP_NAME "gbp-audio-split"
#endif
...
/* ---- SPLIT (Issue #105, §V24): Run B's interleaved variable ----------------------------
 * SPLIT 1: the assignment; SPLIT 3: the floorless sample's store (~0.8 MB). */
static struct gbp_asplit asplit;
static struct gbp_atrace_step tr_sample[GBP_ATRACE_SAMPLE_MAX];

/* SPLIT 2: the spare byte of a production step's record, from the chain's state before and after
 * the call. A call that did no work gets 0. Otherwise: bit 0 the chunk's APPLIED arm (1 = 8-push
 * steps), bit 1 set when the call started the chunk, bits 2..7 the chunk's seq mod 64 -- the seq
 * of the chunk a call works on is `produced` as it was before the call. */
static uint8_t split_tag(int cur0, uint32_t prod0, uint32_t push0)
{
    const int worked = (ap.produced != prod0) || (ap.cur >= 0 && (cur0 < 0 || ap.cur_pushes != push0));
    if (!worked) return 0u;
    return (uint8_t)(((ap.cur_step == GBP_ASPLIT_HALF_PUSHES) ? 1u : 0u) | ((cur0 < 0) ? 2u : 0u) |
                     ((prod0 & 63u) << 2));
}
...
static void live_step(void)
{
    ...
    int sp_cur0;                                                                      /* SPLIT 2 */
    uint32_t sp_prod0, sp_push0;                                                      /* SPLIT 2 */
    ...
    if (live.phase == GBP_ALIVE_WINDOW) {
        ...
        sp_cur0 = ap.cur; sp_prod0 = ap.produced; sp_push0 = ap.cur_pushes;           /* SPLIT 2 */
...
int main(void)
{
    ...
    {                                                                                 /* SPLIT 1, 3 */
        gbp_asplit_init(&asplit);
        ap.step_pushes = gbp_asplit_step_pushes;
        ap.step_pushes_user = &asplit;
        tr.s.sample = tr_sample;
        tr.sample_every = 8u;
    }
     ...
     * else; the verdicts are tools/v22accept.py's. ---- */
    {
        ...
        /* SPLIT 4: the assignment as applied, and the floorless sample */
        ringlog_printf(&rl, "LIVESPLIT seed=%08lx half=%lu full=%lu sample_every=%lu sample=%lu sample_dropped=%lu",
                       (unsigned long)GBP_ASPLIT_SEED, (unsigned long)asplit.chunks_half,
                       (unsigned long)asplit.chunks_full, (unsigned long)tr.sample_every, (unsigned long)tr.sample_n,
                       (unsigned long)tr.sample_dropped);
    }
