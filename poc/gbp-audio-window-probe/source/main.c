/*
 * Open-GBP GBP-AUDIO-001, build stream-0016 — THE AUDIO WINDOW IMAGE (GitHub
 * Issue #59). IMPLEMENTED AND HOST-VALIDATED; it has never touched hardware,
 * nothing here claims that the AUDIO window carries anything, and the run it
 * exists for is pre-registered in HARDWARE_TESTS §V8 and NOT authorised here.
 *
 * WHAT THIS IS. `play-0001` (poc/gbp-play-session) — itself `stream-0015`
 * minus the research instrumentation — plus the ONE thing §V8 needs and no
 * image had: RETENTION of whole runs of consecutive AUDIO blocks around the
 * presses, and EMISSION of them with an anchor a reader can align to the press
 * that caused each one.
 *
 * THE BUILD ID KEEPS THE `stream-` FAMILY because §V8.7 named it and §V8.9
 * reserved the run's raw-file names with it. The service path IS the stream
 * probe's; the directory name says what the image is for.
 *
 * THE INHERITED NAMES ARE KEPT ON PURPOSE. `PLAY_SAFETY_SECONDS`,
 * `PLAY_SLICE_TILE_ROWS`, `session_step()` and the rest keep play-0001's
 * spelling because the functions they appear in are BYTE-IDENTICAL to
 * play-0001's and tests/host/test_awin_image.py diffs them literally. A rename
 * would have made the subtraction unreadable, which is the one thing the
 * subtraction exists to prevent.
 *
 * Everything a run needs is play-0001's, reused with its BEHAVIOUR unchanged:
 *
 ...
 *   - THE INPUT PATH AND THE KEY RECORD: input_step() is BYTE-IDENTICAL to
 *     play-0001's (and so to stream-0015's), and keylog_emit() differs by ONE
 *     LINE -- the anchor, below. tests/host/test_awin_image.py diffs both
 *     against play-0001 and names the line. The descriptor and the policy are
 *     the src/gbp data the routing FACT of HARDWARE_TESTS §V7.4 was
 ...
 *
 * WHAT IS NEW (Issue #59), and it is three insertions and a store:
 *
 *   A. THE ANCHOR, one line in keylog_emit(): awin_note_event(&e), placed
 *      BEFORE the ringlog's own admission test so that the window can never
 *      depend on the log's headroom. It arms a window only on a RISING edge --
 *      a key that went down -- so a release does not consume one of the four.
 *
 *   B. THE GATE, one line in pump(): awin_gate(), which arms §V8.5.1's silent
 *      control once, AWIN_NOT_BEFORE_MS after the CONTROL transform (the same
 *      epoch and the same 5000 ms §V5.55 chose, and for the same reason: the
 *      AGB's own boot is done by then), and raises the end flag when the
 *      capture is complete.
 *
 *   C. THE SINK, one optional field in the service path's config: cfg.awin.
 *      Per received AUDIO block, after the drain and its commit, at most one
 *      4096-byte copy in RAM, measured. With the field NULL -- every earlier
 *      build -- it does not exist as far as the device is concerned.
 *
 *   D. THE STORE: GBP_AWIN_STORE_BYTES = 5 windows x 256 blocks x 4096 B =
 *      5 242 880 B. Four press windows (§V8.3.2) and one control (§V8.5.1).
 *
 * WHAT IS INHERITED FROM Issue #39:
 *
...
#define OPENGBP_APP_NAME "gbp-audio-window-probe"
#endif
...
/* ---- THE SESSION'S BOUNDS, AND THE STORE BUDGET (Issue #59, item 3) ---------
 *
 * None of these is a property of the Game Boy Player; each is a bound of THIS
 * image. play-0001's lesson is why they are written down BEFORE the run: its
 * event store bound the session at ~274 s and nobody had said so in advance
 * (GBP-HW-282). Here every bound is stated, and the one that binds is named.
 *
 * WHAT THE RUN NEEDS. §V8.10 is four presses at least three seconds apart on a
 * cartridge that is already booted: call it 40 s of session with generous
 * slack, not the twelve minutes a play session wants.
 *
 * PLAY_SAFETY_SECONDS   120 s, down from play-0001's 720. THE BOUND THAT FIRES
 *                       IN A RUN THAT DOES NOT COMPLETE ITS CAPTURE, by
 *                       construction: both stores below outlast it. A run that
 *                       completes its four windows ends earlier and as a
 *                       SUCCESS (session_end).
 * PLAY_MAX_DELIVERIES   the u32 delivery guard, above the budget: RUN 17 ran at
 *                       6 314 deliveries/s, so 120 s is ~758 k and 1.5 M sits
 *                       1.98x above it. It binds only if the physical rate
 *                       exceeds ~12 500/s, and then the run ends as
 *                       `delivery_cap`.
 * PLAY_FRAME_RECORDS    16384, THE STORAGE CONTRACT'S MINIMUM (§V5.29: the gate
 *                       honours a larger set and REFUSES a smaller one, which is
 *                       what stream-0002 discovered physically, GBP-HW-136). It
 *                       is also 274 s at 59.727 Hz, so the safety budget fires
 *                       first and this store never bounds the session. Saved
 *                       against play-0001: 28672 x 192 B = 5 505 024 B.
 * PLAY_EVENT_RECORDS    8192, ABOVE the contract's 4096 on purpose. The
 *                       archive's figure for this store is that it fills at the
 *                       FRAME rate (59.81/s, the #45 recomputation), not at the
 *                       ~4/s play-0001 assumed: the contract's 4096 would bound
 *                       the session at 68 s, INSIDE the 120 s budget, and this
 *                       image would have repeated play-0001's mistake of
 *                       discovering its own bound afterwards. 8192 = 137 s.
 *                       Saved against play-0001: 8192 x 64 B = 524 288 B.
 * PLAY_SESSION_END_HOLD_MS  how long Z must be held, continuously, to end the
 *                       session by hand. The capture ending itself is the
 *                       normal success; Z is the operator's escape.
 *
 * WHAT THE 5 242 880 B OF WINDOW COSTS, NAMED (Issue #59 item 3). It is funded
 * out of the two stores above -- 6 029 312 B given up, 5 242 880 B taken -- so
 * the image is 786 432 B SMALLER than play-0001 and nothing else was touched to
 * pay for it. NOTHING WAS ECONOMISED ON THE WINDOW: the four windows and the
 * control are §V8.3.2's and §V8.5.1's, at 256 blocks each, and if the budget
 * had not covered them that would have been a finding to report rather than a
 * window to shrink. WHAT THAT MAKES UNMEASURABLE HERE: a session longer than 120 s, and the
 * research instrumentation play-0001 already did without (the OGBPIDX1
 * witness, the full-frame samples, the VI latch trace, the disposition trace).
 * This image answers §V8's two questions and measures nothing else. */
#define PLAY_SAFETY_SECONDS      120u
#define PLAY_MAX_DELIVERIES      1500000u
#define PLAY_FRAME_RECORDS       16384u
#define PLAY_EVENT_RECORDS       8192u
 ...
 * margin (§V3.24, §V5.7). */
static struct gbp_vstate_frame frame_store[PLAY_FRAME_RECORDS];                    /* 16384 x 192 B = 3.00 MiB */
static struct gbp_vstate_event event_store[PLAY_EVENT_RECORDS];                    /* 8192 x 64 B = 0.50 MiB */
...
/* Issue #59: the same for the EVENT store, because the archive's figure for it
 * is the FRAME rate and not the ~4/s play-0001 assumed. The budget claim of the
 * comment above is a compile-time check here, where it cannot drift. */
_Static_assert(PLAY_EVENT_RECORDS >= PLAY_SAFETY_SECONDS * 60u,
               "the event store must outlast the safety budget at the frame rate");

/* ---- Issue #59: THE AUDIO WINDOW STORE (HARDWARE_TESTS §V8.3.2) ------------
 *
 * 5 windows x 256 blocks x 4096 B. FOUR press windows, because §V8.3.2 froze
 * four presses and 256 blocks; ONE control, because §V8.5.1 makes AU's
 * positive verdict unavailable without blocks from the SAME RUN taken while no
 * tone should be sounding. The size is the pre-registration's and is asserted
 * here against the module's constants so that neither can drift alone. */
static uint8_t awin_store[GBP_AWIN_STORE_BYTES] ATTRIBUTE_ALIGN(32);               /* 5.00 MiB */
static struct gbp_awin awin;
_Static_assert(sizeof awin_store == GBP_AWIN_STORE_BYTES,
               "the audio window store must be exactly §V8.3.2's 5 242 880 bytes");
_Static_assert(GBP_AWIN_BLOCKS == 256u && GBP_AWIN_PRESS_WINDOWS == 4u,
               "§V8.3.2 froze four windows of 256 blocks; a build may not economise");
_Static_assert(GBP_AWIN_BLOCK_SIZE == GBP_VSTATE_AUDIO_BLOCK_SIZE,
               "the window's block must be the AUDIO block the service path drains");
...
/* ---- Issue #59 (A) and (B): the AUDIO WINDOW's two insertions ---------------
 *
 * THE ARM IS A RISING EDGE, NOT A KEY EVENT. gbp_input reports a write on every
 * CHANGE, so one press of A produces two -- the key going down and the key
 * coming up. §V8.3.2 has four windows and §V8.10 asks for four presses, so a
 * release must not consume one: the window is armed only when the new word has
 * a bit the previous word did not. `awin_prev_word` is this file's, written
 * only here, and the FIRST event (the policy's `KEYPAD := 0` with nothing
 * held) arms nothing because it sets no bit.
 *
 * THE CONTROL IS ARMED ONCE, AWIN_NOT_BEFORE_MS after the CONTROL transform.
 * §V8.5.1 wants blocks from the SAME RUN taken while no tone should be
 * sounding, and the AGB's own boot is not that: 5000 ms is the epoch and the
 * value §V5.55 chose for the witness streak, for the same reason, and it is
 * prospective and content-blind here too. If the Operator presses before the
 * gate releases, the press arms a window and the control is refused -- counted,
 * never silent, and §V8.5.2 then reads the run INCONCLUSIVE for lack of a
 * control rather than inventing one.
 *
 * NEITHER TOUCHES THE DEVICE, a clock of its own, the filesystem or the store:
 * the copy is the service path's (cfg.awin), and these two only decide when a
 * window is open. */
#define AWIN_NOT_BEFORE_MS 5000u
static uint16_t awin_prev_word;
static uint32_t awin_presses_seen, awin_releases_seen;
static uint64_t awin_not_before_ticks;
static int awin_control_armed;
static int end_flag;                  /* cfg.session_end: Z, or the capture complete */

static void awin_note_event(const struct gbp_input_event *e)
{
    struct gbp_awin_press p;
    uint16_t rising;
    if (!e) return;
    rising = (uint16_t)(e->word & (uint16_t)~awin_prev_word);
    awin_prev_word = e->word;
    if (!rising) { awin_releases_seen++; return; }
    awin_presses_seen++;
    p.event_n = e->n;
    p.word = e->word;
    p.keys = (uint32_t)e->keys;
    p.t_poll = e->t_poll;
    p.t_attempt = e->t_attempt;
    p.t_done = e->t_done;
    p.t_arm = e->t_done ? e->t_done : e->t_attempt;
    (void)gbp_awin_arm_press(&awin, &p);
}


/* Issue #59: the sidecar's sink and the ONLY transient buffer the save uses --
 * one record. The 5 MiB store is never copied in full: gbp_awindump_stream
 * hands each block to the sink straight out of the window. */
static int sink_sd(void *ctx, const uint8_t *data, uint32_t len)
{
    return sdlog_stream_write((struct sdlog_stream *)ctx, data, len);
}
static uint8_t awin_chunk[GBP_AWINDUMP_CHUNK];
_Static_assert(sizeof awin_chunk >= GBP_AWINDUMP_HEADER_SIZE &&
               sizeof awin_chunk >= GBP_AWINDUMP_RECORD_SIZE,
               "the chunk must hold the header and one record");
...
/* Issue #59 (B): the gate, defined here because it reads `session` above. It
 * arms §V8.5.1's control once and raises the run's success flag; it touches no
 * device, no filesystem and no store. */
static void awin_gate(void)
{
    const struct gbp_transport *t = in_transport;
    if (!awin_control_armed && t && pump_res && pump_res->t_control_transform) {
        const uint64_t now = t->ticks64(t->ctx);
        if (now - pump_res->t_control_transform >= awin_not_before_ticks) {
            awin_control_armed = 1;          /* once, whether or not the arm was refused */
            (void)gbp_awin_arm_control(&awin, now);
        }
    }
    /* The run's SUCCESS: §V8.3.2's capture is complete. Z stays the operator's
     * escape, and either raises the same caller-owned flag. */
    if (!end_flag && (session.end_requested || gbp_awin_complete(&awin))) end_flag = 1;
}
...
int main(void)
{
    ...
    /* Issue #59: the flag the service path reads is THIS image's, raised by the
     * capture completing OR by the operator's Z. play-0001 pointed the config
     * straight at the session's flag; here two sources share one flag, and
     * awin_gate() is the only writer. */
    cfg.session_end = &end_flag;

    /* Issue #59: the AUDIO window. A store fault is a REFUSAL to run, in the
     * same spirit as the state model's first gate (§V5.29): an image that
     * cannot retain the window cannot answer §V8, and finding that out after
     * the Operator's session would waste a physical run. */
    if (gbp_awin_init(&awin, awin_store, (uint32_t)sizeof awin_store) != 0) {
        printf("\n  AWIN store unusable: %s -- NOT RUNNING.\n", gbp_awin_fault(&awin));
        ringlog_printf(&rl, "AWIN fault=%s store_bytes=%lu", gbp_awin_fault(&awin),
                       (unsigned long)sizeof awin_store);
        for (;;) { VIDEO_WaitVSync(); PAD_ScanPads(); if (PAD_ButtonsDown(0) & PAD_BUTTON_START) return 0; }
    }
    awin_not_before_ticks = ((uint64_t)tb_hz * AWIN_NOT_BEFORE_MS) / 1000u;
    cfg.awin = &awin;
    ...
    /* Issue #59: the window's own configuration, as data, before the run */
    ringlog_printf(&rl, "ENVAWIN windows=%lu press_windows=%lu blocks=%lu block_size=%lu store_bytes=%lu "
                        "not_before_ms=%lu not_before_ticks=%llu store=%08lx success=capture_complete_or_Z",
                   (unsigned long)GBP_AWIN_WINDOWS, (unsigned long)GBP_AWIN_PRESS_WINDOWS,
                   (unsigned long)GBP_AWIN_BLOCKS, (unsigned long)GBP_AWIN_BLOCK_SIZE,
                   (unsigned long)GBP_AWIN_STORE_BYTES, (unsigned long)AWIN_NOT_BEFORE_MS,
                   (unsigned long long)awin_not_before_ticks,
                   (unsigned long)MEM_VIRTUAL_TO_PHYSICAL(awin_store));
    ...
    cfg.awin = 0;                       /* Issue #59: and no further block is kept  */
    gbp_awin_finish(&awin);             /* a window still filling is INCOMPLETE, and says so */
    ...
    /* Issue #59: the window, as data. One summary record and one per window, so
     * a reader who has only the log knows which windows exist, what anchored
     * them and what was refused -- before opening 5 MB of sidecar. */
    ringlog_printf(&rl, "AWIN fault=%s complete=%d windows=%lu closed=%lu stored=%lu seen=%lu ignored=%lu failed=%lu "
                        "arms=%lu refused_busy=%lu refused_full=%lu presses=%lu releases=%lu control_armed=%d "
                        "copy_ticks=%lu/%lu/%lu n=%lu units=min/mean/max",
                   gbp_awin_fault(&awin), gbp_awin_complete(&awin), (unsigned long)awin.windows,
                   (unsigned long)awin.windows_closed, (unsigned long)gbp_awin_stored_total(&awin),
                   (unsigned long)awin.blocks_seen, (unsigned long)awin.blocks_ignored,
                   (unsigned long)awin.blocks_failed, (unsigned long)awin.arms,
                   (unsigned long)awin.arm_refused_busy, (unsigned long)awin.arm_refused_full,
                   (unsigned long)awin_presses_seen, (unsigned long)awin_releases_seen, awin_control_armed,
                   (unsigned long)(awin.ticks_n ? awin.ticks_min : 0u),
                   (unsigned long)gbp_awin_ticks_mean(&awin),
                   (unsigned long)awin.ticks_max, (unsigned long)awin.ticks_n);
    {
        unsigned wi;
        for (wi = 0; wi < awin.windows; wi++) {
            const struct gbp_awin_anchor *aw = &awin.anchor[wi];
            ringlog_printf(&rl, "AWINW i=%u kind=%s ord=%lu flags=%04lx blocks=%lu skipped=%lu n=%lu word=%04lx "
                                "t_poll=%llx t_attempt=%llx t_done=%llx t_arm=%llx cyc_first=%llu cyc_last=%llu",
                           wi, (aw->kind == (uint32_t)GBP_AWIN_CONTROL) ? "control" : "press",
                           (unsigned long)aw->ordinal, (unsigned long)aw->flags, (unsigned long)aw->blocks,
                           (unsigned long)aw->skipped, (unsigned long)aw->event_n, (unsigned long)aw->word,
                           (unsigned long long)aw->t_poll, (unsigned long long)aw->t_attempt,
                           (unsigned long long)aw->t_done, (unsigned long long)aw->t_arm,
                           (unsigned long long)aw->first_cycle, (unsigned long long)aw->last_cycle);
        }
    }
    ...
    printf("  AWIN    %s: %lu/%lu windows closed, %lu blocks stored, %lu seen, %lu failed; arms %lu (busy %lu, full %lu); copy %lu/%lu/%lu ticks\n",
           gbp_awin_complete(&awin) ? "CAPTURE COMPLETE" : "INCOMPLETE",
           (unsigned long)awin.windows_closed, (unsigned long)awin.windows,
           (unsigned long)gbp_awin_stored_total(&awin), (unsigned long)awin.blocks_seen,
           (unsigned long)awin.blocks_failed, (unsigned long)awin.arms,
           (unsigned long)awin.arm_refused_busy, (unsigned long)awin.arm_refused_full,
           (unsigned long)(awin.ticks_n ? awin.ticks_min : 0u), (unsigned long)gbp_awin_ticks_mean(&awin),
           (unsigned long)awin.ticks_max);
    {
        unsigned wi;
        for (wi = 0; wi < awin.windows; wi++)
            printf("          w%u %-7s blocks %3lu/%lu%s%s  n=%lu word=%04lx\n", wi,
                   (awin.anchor[wi].kind == (uint32_t)GBP_AWIN_CONTROL) ? "control" : "press",
                   (unsigned long)awin.anchor[wi].blocks, (unsigned long)GBP_AWIN_BLOCKS,
                   (awin.anchor[wi].flags & GBP_AWIN_F_INCOMPLETE) ? "  INCOMPLETE" : "",
                   (awin.anchor[wi].flags & GBP_AWIN_F_GAP) ? "  GAP" : "",
                   (unsigned long)awin.anchor[wi].event_n, (unsigned long)awin.anchor[wi].word);
    }
    ...
    printf("\n  stop=session_end IS THE ONLY SUCCESS, and here it means the CAPTURE COMPLETED (or Z was held).\n");
    printf("  A run that ended any other way did not finish §V8's four windows,\n");
    ...
    printf("\n  X = save log AND the %lu-byte audio sidecar to SD    START = exit    POWER CYCLE REQUIRED\n",
           (unsigned long)((uint64_t)gbp_awin_stored_total(&awin) * GBP_AWIN_BLOCK_SIZE));
    for (;;) {
        VIDEO_WaitVSync();
        PAD_ScanPads();
        if (!saved && (PAD_ButtonsDown(0) & PAD_BUTTON_X)) {
            ...
            char extra[240];
            int rc;
            /* Issue #59: THE SIDECAR FIRST. It is the run's evidence; the log is
             * the account of it. A card that fills has then lost the account and
             * not the bytes, and the console still shows the summary. Written
             * after the teardown, from fixed RAM, through a streaming sink: no
             * filesystem call and no CRC anywhere in the capture path. */
            {
                struct gbp_awindump_info ainfo;
                struct sdlog_stream astream;
                char astat[96] = "";
                uint64_t aw = 0;
                long an = -1;
                int arc = -1;
                ...
                ainfo.tb_hz = tb_hz;
                if (gbp_awindump_set_identity(&ainfo, TEST_ID, OPENGBP_BUILD_ID,
                                              OPENGBP_APP_NAME, OPENGBP_GIT_COMMIT) != 0) {
                    snprintf(astat, sizeof astat, "audio sidecar not written: an identity does not fit");
                } else if (sdlog_stream_open(&astream, TEST_ID, OPENGBP_BUILD_ID, "-audio.bin",
                                             astat, sizeof astat) != 0) {
                    /* astat already explains */
                } else {
                    an = gbp_awindump_stream(&ainfo, &awin, awin_chunk, sizeof awin_chunk,
                                             sink_sd, &astream, &aw);
                    arc = sdlog_stream_close(&astream, astat, sizeof astat);
                    if (an < 0 && arc == 0) arc = -5;
                }
                printf("  SAVE audio   %s   (OGBPAW1 v%u, %lu windows, %lu blocks, %llu B)\n", astat,
                       (unsigned)GBP_AWINDUMP_VERSION, (unsigned long)ainfo.windows_n,
                       (unsigned long)ainfo.blocks_stored, (unsigned long long)aw);
                snprintf(line, sizeof line,
                         "OPENGBP-PLAY SAVEAUDIO rc=%d n=%ld written=%llu flags=%04lx crc=%08lx\n",
                         arc, an, (unsigned long long)aw, (unsigned long)ainfo.flags,
                         (unsigned long)ainfo.total_crc32);
                gecko_puts(line);
                ringlog_printf(&rl, "AWINSAVE rc=%d n=%ld written=%llu flags=%04lx header_crc=%08lx total_crc=%08lx status=%s",
                               arc, an, (unsigned long long)aw, (unsigned long)ainfo.flags,
                               (unsigned long)ainfo.header_crc32, (unsigned long)ainfo.total_crc32, astat);
            }
            /* The log names what it is: the sidecar by name, and the one success. */
            snprintf(extra, sizeof extra,
                     "libogc=%s gecko=%d power_cycle_required=%d sidecar=OGBPAW1 time_target=disabled safety_s=%lu "
                     "session_end=capture_or_Z hold_ms=%lu stop=%s status=%s witness=none awin_complete=%d",
                     _V_STRING, gecko_present, res.power_cycle_required,
                     (unsigned long)PLAY_SAFETY_SECONDS, (unsigned long)PLAY_SESSION_END_HOLD_MS,
                     gbp_vstate_stop_name(res.stop), res.status_name, gbp_awin_complete(&awin));
