                          ...
                          const struct gbp_xfer_info *info, const uint8_t data[GBP_BLOCK_SIZE])
{
    ...
    ringlog_hex(hex, sizeof hex, data, GBP_BLOCK_SIZE);
    if (idx == GBP_IDX_CONTROL) {
        ringlog_printf(log, "RAW %s idx=%x addr=%08lx rc=%s ticks=%lu polls=%u dspcr=%04x sem_vote=%02x sem_b1f=%02x data=%s",
                       ...
                       (unsigned)gbp_majority_vote_byte(data), (unsigned)data[GBP_BLOCK_SIZE - 1u],
                       rc == GBP_OK ? hex : "-");
        ...
        ringlog_printf(log, "RAW %s idx=%x addr=%08lx rc=%s ticks=%lu polls=%u dspcr=%04x sem_disc=%04x sem_gbi=%04x data=%s",
                       tag, idx, (unsigned long)addr, gbp_status_name(rc), (unsigned long)info->ticks,
                       (unsigned)info->polls, (unsigned)info->dma_status,
                       (unsigned)gbp_irq_value_disc(data), (unsigned)gbp_irq_value_gbi(data),
                       rc == GBP_OK ? hex : "-");
    } else {
                                 ...
                                 uint32_t base, unsigned idx, uint8_t out[GBP_BLOCK_SIZE], unsigned *errors)
{
    ...
    gbp_rawlog_log_block(log, tag, addr, idx, rc, &info, out);
    return rc;
}

void gbp_rawlog_log_pi(struct ringlog *log, const char *tag, const char *rc_name, uint32_t intsr, uint32_t intmr)
{
    if (strcmp(rc_name, "ok") != 0) {
        ringlog_printf(log, "PI %s rc=%s", tag, rc_name);
        return;
    }
                       ...
                       uint32_t *intsr, uint32_t *intmr)
{
    gbp_status rc;
    if (!t->read_pi) {
        gbp_rawlog_log_pi(log, tag, "unavailable", 0, 0);
        return 0;
    }
    rc = t->read_pi(t->ctx, intsr, intmr);
    if (rc != GBP_OK) {
        gbp_rawlog_log_pi(log, tag, gbp_status_name(rc), 0, 0);
        return 0;
    }
    gbp_rawlog_log_pi(log, tag, "ok", *intsr, *intmr);
    return 1;
}
