 ...
 * stimulus, OGBPCOORD1 (HARDWARE_TESTS §V6.6, GitHub Issue #7).
 *
 * THIS PROGRAM IS A MEASURING INSTRUMENT. Every pixel it writes is known by
 * construction to an offline model (tools/icoord.py), and nothing here may be
 * "improved" without changing that contract first.
 *
 * WHAT IT IS FOR
 ...
 *
 * BUDGET, static (§V6.17 item 4): the heaviest frame publishes 56 x 160 strip
 *   words + 48 x 80 digit words + 48 x 8 square words = 13 184 words, against
 *   indexed-0003's 128 x 160 = 20 480 words per frame, which published to
 *   VCOUNT 203 (VMARGIN 24) on hardware. The digit rows come from EWRAM (about
 *   twice the DMA cost of IWRAM), which still leaves the estimate below that
 *   measured envelope. The estimate is not evidence; STATUS is.
 *
...
/* ---- PREPARE / PUBLISH ------------------------------------------------------
 * PREPARE runs in the visible period and touches no VRAM; PUBLISH runs in
...
/* Large constant / per-appearance tables live in EWRAM (.sbss: NOLOAD, so
 * they cost no ROM bytes): the digit rows are written once per appearance in
 * PREPARE, the erase rows once at boot, both before any publish reads them. */
#ifdef AGB_HOST_TEST
#define EWRAM_SECTION
#else
#define EWRAM_SECTION __attribute__((section(".sbss")))
#endif
static u16 glyph_rows[GLYPH_ROWS][GLYPH_SPAN_W] EWRAM_SECTION;
...
static void prepare_frame(u32 frame_id, u8 status, const struct sched *sc)
{
    ...
    /* the digit: painted on the entry frame, erased on the exit frame */
    op_digit = OP_NONE;
    if (sc->k >= 1u && sc->phase == 0u) {
        op_digit = OP_PAINT;
        for (r = 0; r < GLYPH_ROWS; r++)
            for (c = 0; c < GLYPH_SPAN_W; c++)
                glyph_rows[r][c] = (c >= 1u && c <= GLYPH_COLS && glyph_lit(sc->digit, c - 1u, r))
...
static void publish_frame(void)
{
    ...
    if (op_digit == OP_PAINT) {
        for (r = 0; r < GLYPH_ROWS; r++)
            pub_copy32(VRAM + (GLYPH_Y0 + r) * SCREEN_W + GLYPH_SPAN_X0, glyph_rows[r], GLYPH_SPAN_W);
