            ...
            if fx_path.endswith(".gbpreplay"):
                ...
                if "\nB " in text:
                    ...
                    self.assertIn("# BLOCKS=", text[:4096], fx_path)
                        # three physical fixtures carry whole-block reads today: GBP-AV-SERVICE-001
                    # (one AUDIO + one VIDEO block), GBP-VIDEO-001 (88 VIDEO + 144 AUDIO reads, of
                    # which only 9 AUDIO payloads were preserved by design) and GBP-VIDEO-002,
                    # whose script is only the prefix the log records: the 003A stage, the FOUR
                    # verify cycles (8 block reads, none of them payload-backed) and the teardown.
                    # Its 51746 lean cycles kept no per-delivery record by design, so they are not
                    # in the script and are not invented.
                    self.assertIn(fx_path, (PHYSICAL_AVSVC, PHYSICAL_VIDEO, PHYSICAL_VSTATE), fx_path)
