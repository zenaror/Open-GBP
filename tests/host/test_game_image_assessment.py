...
class TheAssessmentIsRecordedAndNothingWasBuilt(unittest.TestCase):
    ...
    def test_nothing_was_built_and_nothing_under_the_untouchable_paths_changed(self):
        ...
        # its second attempt for §V26, and Issue #117 the latency round's for §V27
        self.assertEqual(sorted(p for p in os.listdir(os.path.join(ROOT, "poc")) if os.path.isdir(os.path.join(ROOT, "poc", p))),
                         ["audio-output-replay", "gbp-audio-drain-probe", "gbp-audio-game", "gbp-audio-game2", "gbp-audio-live", "gbp-audio-split", "gbp-audio-sync", "gbp-audio-trace", "gbp-audio-window-probe",
        ...
        if not guards.base_available(BASE_COMMIT):
            self.skipTest("the base commit %s is not in this checkout, so the freeze cannot be checked here" % BASE_COMMIT)
        changed = guards.changed_between(BASE_COMMIT, guards.CHECKPOINTS_CLOSED_AT, ["src", "poc", "tools", "Makefile", "stimulus", "captures/fixtures", "docs/protocol", "docs/hardware", "docs/research/HARDWARE_TESTS.md", "docs/research/EVIDENCE.md", "docs/research/UNKNOWNS.md"])   # Issue #29: tracked AND untracked, one implementation
