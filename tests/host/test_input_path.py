...
class TheStaticAttemptIsStatedOnceAndStaysOpen(unittest.TestCase):
    ...
    def test_the_findings_use_the_keypad_namespace_and_mint_no_hardware_id(self):
        ...
        self.assertEqual(hw, 300)   # GBP-HW-261…265: RUN 14 / RUN 15 (Issue #24); 266…271: RUN 16 / 17 / 18 (Issue #33)   # 272: Issue #46 (the CONTROL bit 0x02 split, FACT for the split / HYPOTHESIS for the cause); 273…277: Issue #47 (RUN 23 / RUN 24, §V7.7); 278…284: Issue #52 (RUN 21 / 22 / 25 / 26, §V7.8)   # 285…294: Issue #62 (RUN 30 ingested, §V8.13)   # 295…300: Issue #67 (RUN 31 ingested, §V9.15)
        ...
        # trail names them, which is what a trail is for. The sentinel moves to the next free id.
            ...
            self.assertNotRegex(read(p), r"GBP-HW-30[1-9]|GBP-HW-3[1-9]\d|GBP-HW-[4-9]\d\d")
