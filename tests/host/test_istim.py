...
class Geometry(unittest.TestCase):
    ...
    def test_the_strips_are_exactly_the_payload_width(self):
        ...
        self.assertEqual(istim.STRIP_BITS, 8 + 24 + 6 + 8)
    ...
    def test_the_pixel_function_is_total(self):
        for f in (0, 1, 0x7FFFFF, 0xFFFFFF):
            for y in (0, 1, 79, 158, 159):
                for x in (0, 1, 46, 47, 48, 191, 192, 193, 238, 239):
...
class Crc8(unittest.TestCase):
    ...
    VECTORS = {
        (0x000000, 0): 0xF6, (0x000000, 39): 0x03,
        (0x000001, 0): 0x31, (0x000001, 39): 0xC4,
        (0xFFFFFF, 0): 0x3F, (0xFFFFFF, 39): 0xCA,
        (0x123456, 17): 0xDC,
    }

    def test_the_known_answer_vectors(self):
        for (f, b), want in self.VECTORS.items():
            self.assertEqual(istim.crc8_payload(f, b), want,
                             "f=0x%06x b=%d" % (f, b))

    def test_the_payload_is_thirty_bits_msb_first(self):
        bits = istim.payload_bits(0xA5A5A5, 0b100101)
        self.assertEqual(len(bits), 30)
        self.assertEqual(bits[:24], [int(c) for c in format(0xA5A5A5, "024b")])
        self.assertEqual(bits[24:], [1, 0, 0, 1, 0, 1])

    def test_a_single_bit_flip_in_the_payload_always_changes_the_crc(self):
        """A CRC-8 cannot miss a single-bit error in a 30-bit message."""
        for f, b in ((0, 0), (0x0F0F0F, 13), (0xFFFFFF, 39)):
            base = istim.payload_bits(f, b)
...
class ModularClassification(unittest.TestCase):
    ...
    def test_the_five_classes(self):
        ...
        self.assertEqual(istim.classify_delta(10, 11), istim.CONTIGUOUS)
        self.assertEqual(istim.classify_delta(10, 12), istim.FORWARD_GAP)
        self.assertEqual(istim.classify_delta(10, 10 + (1 << 23) - 1), istim.FORWARD_GAP)
        self.assertEqual(istim.classify_delta(10, 10 + (1 << 23)), istim.UNRESOLVED_HALF_RANGE)
        self.assertEqual(istim.classify_delta(10, 10 + (1 << 23) + 1), istim.REORDER_BACKWARD)
        self.assertEqual(istim.classify_delta(10, 9), istim.REORDER_BACKWARD)

    def test_the_wrap_is_contiguous(self):
        seq = [0xFFFFFE, 0xFFFFFF, 0x000000, 0x000001]
        for a, b in zip(seq, seq[1:]):
            self.assertEqual(istim.classify_delta(a, b), istim.CONTIGUOUS,
                             "0x%06x -> 0x%06x" % (a, b))

    def test_the_half_range_is_never_called_a_gap(self):
        self.assertNotEqual(istim.classify_delta(0, 1 << 23), istim.FORWARD_GAP)
...
class ContentPattern(unittest.TestCase):
    ...
    def test_the_phase_multiplier_is_coprime_with_the_period(self):
        ...
        self.assertEqual(len(phases), istim.BAR_PERIOD,
                         "40 blocks over a 35-phase bar: 35 distinct phases is the maximum")
...
class Ddr2SignatureFeasibility(unittest.TestCase):
    ...
    def test_the_strip_contributes_a_constant_to_the_signature(self):
        ...
        self.assertEqual(ref, (92, 92))

    def test_the_signature_has_period_35_in_the_frame_id(self):
        for b in (0, 1, 19, 39):
            for f in range(0, 12):
                self.assertEqual(istim.vsig_block_fast(f, b),
