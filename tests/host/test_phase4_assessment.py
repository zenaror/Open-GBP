...
class PromotionIsTraceableAndMintsNothing(unittest.TestCase):
    ...
    def test_no_new_evidence_or_finding_id(self):
        ...
        self.assertEqual((hw, vid), (311, 35))   # 285…294: Issue #62 (RUN 30 ingested, §V8.13)   # Issue #33 (RUN 16 / 17 / 18, §V7.4) minted GBP-HW-266..271;   # 295…300: Issue #67; 301…302: #67's validation (the rate/layout split, U-GBP-039's probe); 303…307: #72, RUN 32; 308…311: #78, RUN 33 and RUN 34
        ...
        # trail names them, which is what a trail is for. The sentinel moves to the next free id.
            ...
            self.assertNotRegex(read(p), r"GBP-HW-31[2-9]|GBP-HW-3[2-9]\d|GBP-HW-[4-9]\d\d|GBP-VID-03[6-9]|GBP-VID-0[4-9]\d", p)
