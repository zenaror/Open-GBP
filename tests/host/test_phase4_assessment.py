...
class PromotionIsTraceableAndMintsNothing(unittest.TestCase):
    ...
    def test_no_new_evidence_or_finding_id(self):
        ...
        self.assertEqual((hw, vid), (272, 35))   # Issue #33 (RUN 16 / 17 / 18, §V7.4) minted GBP-HW-266..271;
                                                 # Issue #46 minted GBP-HW-272 (the CONTROL bit 0x02 split)
        ...
        for p in PROMOTED + [ROADMAP, HANDOFF]:
            self.assertNotRegex(read(p), r"GBP-HW-27[3-9]|GBP-HW-2[8-9]\d|GBP-HW-[3-9]\d\d|GBP-VID-03[6-9]|GBP-VID-0[4-9]\d", p)
