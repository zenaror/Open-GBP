...
class TheDocumentsAndTheFreeze(unittest.TestCase):
    ...
    def test_the_evidence_rows_exist_once_and_carry_the_identities(self):
        ...
        self.assertEqual(max(int(n) for n in re.findall(r"^#{2,4} +GBP-HW-(\d{3})\b", ev, re.M)), 312)   # GBP-HW-266…271: RUN 16 / 17 / 18 (Issue #33)   # 285…294: Issue #62 (RUN 30 ingested, §V8.13)   # 295…300: Issue #67 (RUN 31 ingested, §V9.15); 301…302: #67's validation (the rate/layout split, U-GBP-039's probe); 303…307: #72, RUN 32; 308…311: #78, RUN 33 and RUN 34; 312: #79, duty()'s mechanism
