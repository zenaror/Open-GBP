...
class TopologyNamesGateAndProcedure(unittest.TestCase):
    ...
    def test_the_ten_names_once_in_the_part_once_in_the_handoff_none_on_disk_none_beyond(self):
        ...
        self.assertEqual(len(re.findall(r"captures/local/\S*run(?:3[8-9]|[4-9]\d)\S*", guards.at_close("docs/research/HARDWARE_TESTS.md"))), 0)   # run21 / run22: Issue #41, §V7.6; run30: Issue #58, §V8 (GBP-AUDIO-001, reserved and not on disk); run31: Issue #64, §V9 (GBP-AUDIO-002, reserved and not on disk); run32: nothing beyond RUN 31 exists (Issue #67); run33: Issue #75, §V13 (GBP-AUDIO-004, reserved and not on disk); run34: §V14 (GBP-AUDIO-003 repeated, reserved and not on disk); run35: §V16.5 (rides along, reserved)   # run36 / run37: Issue #90 (RUN 36 ingested, §V21.9) and #91
...
class NothingElseMoved(unittest.TestCase):
    ...
    def test_nothing_under_the_untouchable_paths_changed_against_the_base(self):
        ...
        changed = guards.changed_between(BASE_COMMIT, guards.CHECKPOINTS_CLOSED_AT, ["src", "poc", "tools", "Makefile", "stimulus", "captures/fixtures", "docs/protocol", "docs/hardware", "docs/research/EVIDENCE.md", "docs/research/UNKNOWNS.md"])   # Issue #29: tracked AND untracked, one implementation
