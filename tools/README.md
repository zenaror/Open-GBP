...
| `probelog.py` | Parse device logs (SD file or USB Gecko capture): `parse` → JSON, `fixture` → replay script for `src/gbp/gbp_replay.c` (`--note` adds comment lines, used to mark synthetic scripts; GBP-INIT-003A records `IRQW`/`CTLW t_after=`/`SNAP tag=EVENT poll_intsr=`/`WINDOW t_end=` become `W`+`T`, `T`+`P p`, `T`; GBP-INIT-004 records `PREPARE gen=` → optional `I p`, `REARM t_rearm=` → `T`, `NEXTCAUSE found=0 t_end=` → `T`, the per-cycle 003B records with ` n=`/`-n`; GBP-INIT-003B records `HANDLERPI intsr_at_entry=…`/`HANDLERPI2` extend the `I u` line by five numbers and `MAINPICLEANUP … performed=1` becomes `P a`; record kinds may carry a digit; GBP-VIDEO-001's compact per-cycle records `CYCU`/`CYCW`/`CYCH`/`CYCD`/`CYCR` and `ADMIT` become the same operation stream a verify cycle's detailed records produce, and a verify cycle's compact records are skipped so no operation is emitted twice), `check` → interpretation of TEST/RAW records |
| `avseq.py` | Parse the sequence sidecar of GBP-VIDEO-001 (`OGBPSEQ1`, format 1, `src/gbp/gbp_avseqdump.h`): `info`, `cycles`, `video`, `audio`, `boundaries`, `frames`, `extract`, `json`, and `oracle` — the **only** content oracle of the experiment. The probe never compares a captured block with anything on the console; this tool does it offline, reading the private references (the Disc's embedded idle frame at `0x801B45A0`, GBI's two 40-entry checksum tables) from `input/extracted/` at run time and storing none of them. Without those inputs the verdict is `reference_content=unavailable`, never a failure of the run. GBI's per-block checksum (low 32 bits plus the carry count) is anchored to the physical GBP-AV-SERVICE-001 block, whose value `0x7F0FFF10` is entry 0 of both tables |
| `vstate.py` | Parse the sidecar of GBP-VIDEO-002 (`OGBPSEQ1`, format **2**, `src/gbp/gbp_vstatedump.h`): `info`, `frames`, `events`, `episodes`, `cycles`, `intervals`, `signatures`, `extract`, `json`, and `oracle` — the **only** content oracle of that experiment. Format 1 and format 2 share the magic and the footer but not the header size, and each parser checks `version` and `header_size` before anything else, so neither can read the other. `oracle` classifies each PRESERVED episode against the Disc's embedded frame and GBI's two tables, read from `input/extracted/` at run time and stored nowhere; without them the verdict is `reference_content=unavailable`, never a failure of the run. The per-block checksum is the same function the runtime computes, so a physical capture and this tool are directly comparable |
| `isr_audit.py` | Static audit of a one-shot PI HSP handler as linked (`--symbol`, default `hsp_backend_oneshot_isr`; `hsp_backend_oneshot_isr_ext` for GBP-INIT-003B; `hsp_backend_oneshot_isr_multi` for GBP-INIT-004, whose entry sequence GCC duplicates into two paths — two `__MaskIrq` calls, still one INTSR store): only `__MaskIrq` callable, no indirect call, no denied symbol, PI base and time base referenced, exactly one store to PI INTSR (value 0x2000, after the `__MaskIrq` call), no store to PI INTMR, loops allowed, instruction count reported; register values from `poc_audit.track_registers`; `make initirq-audit` / `make initirqb-audit` |
| `poc_audit.py` | Static audit of every object linked into a POC, per profile (`--profile 003a`, default: PI HSP masked for the whole run — no interrupt-path object/symbol, INTSR store only in `h_write_intsr`, 3 IRQ-register write sites; `--profile 004`: `hsp_backend_irq_multi.o` required and `hsp_backend_irq.o` forbidden, one `__UnmaskIrq` site, 3 + 1 + 1 IRQ-register write sites (stage, service ACK, probe REARM), the multi-cycle handler's two mask sites pinned; `--profile 003b`: `hsp_backend_irq.o` required, `__UnmaskIrq` from `h_irq_unmask` only, `IRQ_Request` from the install/restore pair only, `__MaskIrq` from the mask primitive and the two handlers only, INTSR stores in those three functions only, 3 + 1 IRQ-register write sites, `main.o` uses the ext constructor), stores to PI INTMR always forbidden (register values tracked by a forward data-flow pass over each function's control-flow graph: `lis`/`ori`/`addi`…, branch targets merge by agreement, loops converge, calls clobber the volatile GPRs, relocated immediates unknown), CONTROL write call sites (2), ELF symbol presence; `make initirqa-audit` / `make initirqb-audit` / `make avsvc-audit` / `make video-audit`; `--profile video` (GBP-VIDEO-001) requires the sequence core, the `OGBPSEQ1` sidecar and the repeated-service probe, forbids the AVSVC probe and the v2 sidecar, pins the QUIET delivery as the only one the probe calls and six IRQ-register write sites |
